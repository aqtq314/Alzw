...
type private ByteStringIndexer (index) =
    member val Index : int = index with get, set
    member val HasChildren : bool = false with get, set

type AlzwCoder (blockSize, forbiddenRegion) =
    static let [<Literal>] rangeCoderPrecision = 28

    let stepRangeList =
        let rangeList = MultiRangeList ()
        rangeList.Add 1                           // EOF
        rangeList.Add (forbiddenRegion : int)     // Forbidden region
        rangeList.Add (10000 - rangeList.Extent)  // Alphabet
        rangeList

    member x.Encode (bytes : byte []) =
